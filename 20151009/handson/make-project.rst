...
Sphinxドキュメントの作成を始めるには、最初に `プロジェクト` を作成します。

.. figure:: images/html_introduction.png

   プロジェクトの構成とSphinxのコマンドの関係

`プロジェクト` はSphinxでドキュメントを扱う上での1つの単位で、最初のreST原稿のひな形と設定ファイル、ディレクトリで構成されます。プロジェクトはSphinxのコマンド :command:`sphinx-quickstart` を実行し、質問に答えていくことで作成されます。まずはプロジェクトを作るところから順番にやってみましょう。
...
それでは、次に、 :doc:`Sphinxのドキュメントをパワーアップさせる <use-markup>` 、各種マークアップやディレクティブを紹介します。
